...
void CAchievementsDialog_XBox::ApplySettings( KeyValues *pResourceData )
{
	...
	{		
		IAchievement* pCurAchievement = (IAchievement*)achievementmgr->GetAchievementByIndex( i, 0 );
...
{
	// Handle close here, CBaseModPanel parent doesn't support "DialogClosing" command
...
CAchievementsDialog::CAchievementsDialog(vgui::Panel *parent) : BaseClass(parent, "AchievementsDialog")
{
	...
	if ( achievementmgr )
	{
		...
		{		
			IAchievement* pCur = achievementmgr->GetAchievementByIndex( i, 0 );
